...
namespace WebApplication1.Repositories.Impl
{
    public class Repository : IRepository
    {
        ...
        public async Task<IEnumerable<HCNRH>> GetByTwoKeyWithTimeForHCNRH(string bhno, string cseq, string sdate, string edate, string stockSymbol)
        {
            ...
            //    Logger.Log(1, "參數", $"Record: CSEQ={hcnrh.CSEQ}, BHNO={hcnrh.BHNO}, TDATE={hcnrh.TDATE}, SDateComparison={compareSDate}, EDateComparison={compareEDate}, MatchesCSEQ={matchesCSEQ}, MatchesBHNO={matchesBHNO}, DateInRange={dateInRange}, StockMatch={stockMatch}");

            //    if (!(matchesCSEQ && matchesBHNO && dateInRange && stockMatch))
            //    {
            //        Logger.Log(1, "參數", $"Record did not match query: CSEQ={hcnrh.CSEQ}, BHNO={hcnrh.BHNO}, TDATE={hcnrh.TDATE}");
            //    }
            //}
